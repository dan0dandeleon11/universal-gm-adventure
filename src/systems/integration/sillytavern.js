...
import { runPostProcessingAgents } from '../pipeline/index.js';
...
export async function onMessageReceived(data) {
    ...
    // Pipeline: run post-processing agents (world state tracking, etc.)
    if (extensionSettings.pipeline?.enabled) {
        const lastMsg = chat?.[chat.length - 1];
        if (lastMsg && !lastMsg.is_user) {
            runPostProcessingAgents(lastMsg.mes || '').catch(err =>
                console.error('[RPG Companion] Pipeline post-processing failed:', err)
            );
        }
    }
