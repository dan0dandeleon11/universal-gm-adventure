...
// Summarizer import
import { maybeRefreshSummary } from '../summarizer/index.js';

// Pipeline imports
import { runPreGenerationAgents, startParallelAgents } from '../pipeline/index.js';

// Data system imports for context injection
import { formatWorldStateForInjection } from '../agents/worldState.js';
import { getRelevantNPCs, formatNPCsForInjection } from '../npcs/index.js';
import { formatSessionsForInjection, incrementTurnCount } from '../sessions/index.js';
import { formatCampaignForInjection } from '../campaign/index.js';
import { formatMapForInjection } from '../maps/index.js';
import { formatSceneForInjection } from '../scene/index.js';
import { formatNPCResponsesForInjection } from '../agents/npcRouter.js';
...
export async function onGenerationStarted(type, data, dryRun) {
    ...
    // Summarizer: refresh rolling summary and inject at configured depth
    try {
        await maybeRefreshSummary();
    } catch (error) {
        console.error('[RPG Companion] Summary refresh failed:', error);
    }

    // GM Mode: inject unified context if enabled (combines tracker + GM + summary)
    if (isUnifiedModeEnabled()) {
        try {
            await injectUnifiedContext();
        } catch (error) {
            console.error('[RPG Companion] Unified context injection failed:', error);
        }
    }

    // Pipeline: run pre-generation agents and start parallel agents
    if (extensionSettings.pipeline?.enabled && !shouldSuppress) {
        try {
            await runPreGenerationAgents();
            startParallelAgents();
        } catch (error) {
            console.error('[RPG Companion] Pipeline pre-gen failed:', error);
        }
    }

    // Data system context injection (world state, NPCs, sessions, campaign, maps, scene)
    if (!shouldSuppress) {
        try {
            const dataBlocks = [];
            if (extensionSettings.worldStateAgent?.enabled && extensionSettings.worldStateAgent?.injectAsContext) {
                const ws = formatWorldStateForInjection();
                if (ws) dataBlocks.push(ws);
            }
            if (extensionSettings.npcSystem?.enabled && extensionSettings.npcSystem?.injectRelevant) {
                const recentTexts = (chat || []).slice(-5).map(m => m.mes || '');
                const relevant = getRelevantNPCs(recentTexts);
                const npcXml = formatNPCsForInjection(relevant);
                if (npcXml) dataBlocks.push(npcXml);
            }
            if (extensionSettings.sessions?.enabled) {
                const sessXml = formatSessionsForInjection();
                if (sessXml) dataBlocks.push(sessXml);
                incrementTurnCount();
            }
            if (extensionSettings.campaign?.enabled) {
                const campXml = formatCampaignForInjection();
                if (campXml) dataBlocks.push(campXml);
            }
            if (extensionSettings.maps?.enabled) {
                const mapXml = formatMapForInjection();
                if (mapXml) dataBlocks.push(mapXml);
            }
            if (extensionSettings.sceneModel?.enabled) {
                const sceneXml = formatSceneForInjection();
                if (sceneXml) dataBlocks.push(sceneXml);
            }
            if (extensionSettings.npcRouter?.enabled && extensionSettings.npcRouter?.injectResponses !== false) {
                const npcRespXml = formatNPCResponsesForInjection();
                if (npcRespXml) dataBlocks.push(npcRespXml);
            }
            if (dataBlocks.length > 0) {
                setExtensionPrompt('rpg-companion-data-systems', dataBlocks.join('\n'), extension_prompt_types.IN_CHAT, 1, false);
            } else {
                setExtensionPrompt('rpg-companion-data-systems', '', extension_prompt_types.IN_CHAT, 1, false);
            }
        } catch (error) {
            console.error('[RPG Companion] Data system injection failed:', error);
        }
    }
