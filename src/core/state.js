...
export let extensionSettings = {
    ...
    // ========================================
    // GM MODE SETTINGS (Universe GM Integration)
    // ========================================
    gmMode: {
        enabled: false, // Master toggle for GM mode features
        activeFramework: 'generic', // Currently selected framework ID

        // Dice settings
        dice: {
            enabled: true,
            defaultDice: 'd20',
            autoRollOnSend: false, // Auto-roll when sending messages
        ...
        // GM API settings (separate from tracker external API)
        api: {
            useMainApi: false, // If true, use SillyTavern's configured API
            provider: 'custom', // 'moonshot', 'glm', 'custom'
            settings: {
                moonshot: {
                    endpoint: 'https://api.moonshot.ai/v1/chat/completions',
                    model: 'moonshot-v1-8k',
                    temperature: 1.0 // Moonshot requires exactly 1.0
                },
                glm: {
        ...
        // Injection settings - how GM rulings are injected for main AI
        injection: {
            mode: 'ambient', // 'ambient' (data only) | 'instructional' | 'disabled'
            position: 'before_user_message', // 'before_user_message' | 'after_user_message' | 'system_prompt'
            depth: 1, // Depth for setExtensionPrompt (1 = before last message)

            // Editable templates with placeholders
            templates: {
                // GM ruling injection template
...
{{/if}}
                ...
                // Tracker context injection template (replaces RPG Companion's default)
                trackerContext: `<world_state>
{{tracker_summary}}
</world_state>`,

                // Combined context template
                fullContext: `{{#if gm_ruling}}{{gm_ruling}}{{/if}}
{{#if tracker_context}}{{tracker_context}}{{/if}}`
            },

            // Custom tag names (users can rename tags)
            customTags: {
                gmRuling: 'ambient_observation',
                trackerContext: 'world_state',
        ...
        // Framework-specific settings stored per framework
        frameworkSettings: {}
    }
};
...
/**
 * GM Mode state - pending GM roll for the next message
 */
export let pendingGMRoll = null;

/**
 * Latest GM narration (for context injection)
 */
export let latestGMNarration = null;

/**
 * GM narration history (keeps last 5)
 */
export let gmNarrationHistory = [];
...
// ========================================
// GM MODE STATE SETTERS
// ========================================

export function setPendingGMRoll(roll) {
    pendingGMRoll = roll;
}

export function getPendingGMRoll() {
    return pendingGMRoll;
}

export function clearPendingGMRoll() {
    pendingGMRoll = null;
}

export function setLatestGMNarration(narration) {
    latestGMNarration = narration;
}

export function getLatestGMNarration() {
    return latestGMNarration;
}

export function addGMNarrationToHistory(entry) {
    gmNarrationHistory.push(entry);
    // Keep only last 5
    if (gmNarrationHistory.length > 5) {
        gmNarrationHistory = gmNarrationHistory.slice(-5);
    }
}

export function getGMNarrationHistory() {
    return [...gmNarrationHistory];
}

export function clearGMNarrationHistory() {
    gmNarrationHistory = [];
    latestGMNarration = null;
}

export function isGMModeEnabled() {
    return extensionSettings.gmMode?.enabled === true;
}

export function getGMSettings() {
    return extensionSettings.gmMode || {};
}
