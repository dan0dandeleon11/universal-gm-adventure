...
export let extensionSettings = {
    ...
    gmMode: {
        enabled: false,
        activeFramework: 'generic',
        unifiedMode: false,

        // GM API connection (separate from main chat)
        api: {
            useMainApi: false,
            provider: 'custom',
            settings: {
                custom: { endpoint: '', model: '', temperature: 0.8 },
                moonshot: { endpoint: 'https://api.moonshot.cn/v1/chat/completions', model: 'moonshot-v1-8k', apiKey: '', temperature: 0.8 },
                glm: { endpoint: 'https://open.bigmodel.cn/api/paas/v4/chat/completions', model: 'glm-4-flash', apiKey: '', temperature: 0.8 }
            },
            maxTokens: 1000
        },
        // Tracker API (can share GM connection or use separate)
        trackerApi: {
            useGmConnection: true,
            provider: 'custom',
            settings: {
                custom: { endpoint: '', model: '', temperature: 0.3 }
            },
            maxTokens: 500
        },
        // Summary API (can share Tracker connection or use separate)
        summaryApi: {
            useTrackerConnection: true,
            provider: 'custom',
            settings: {
                custom: { endpoint: '', model: '', temperature: 0.3 }
            },
        ...
        // Custom frameworks (user-created)
        customFrameworks: {},
    ...
    memoryRecollection: {
        ...
        batchSize: 10,
        autoInterval: 0,
        lorebookName: 'Memory Recollection'
    },
    ...
    narrativeDirector: {
        ...
        pushStoryArmed: false
    },

    // === World State Agent settings ===
    worldStateAgent: {
        enabled: false,
        trackedFields: ['date', 'time', 'location', 'weather', 'temperature', 'characters'],
    ...
    sceneModel: {
        ...
        provider: 'custom',
        settings: {
            custom: { endpoint: '', model: '', temperature: 0.7 }
        },
        maxTokens: 500,
...
/**
 * GM Mode runtime state (not persisted)
 */
...
export function setLatestGMNarration(narration) {
    ...
    if (narration) {
        gmNarrationHistory.unshift(narration);
        if (gmNarrationHistory.length > 5) {
            gmNarrationHistory.pop();
        }
    }
}
...
export function addGMNarrationToHistory(narration) {
    if (narration) {
        gmNarrationHistory.unshift(narration);
        if (gmNarrationHistory.length > 20) {
            gmNarrationHistory.pop();
        }
    }
}
...
/**
 * Rolling summary state (not persisted — rebuilt from chat metadata)
 */
export let rollingSummaryData = null;

export function setRollingSummaryData(data) {
    rollingSummaryData = data;
}

export function getRollingSummaryData() {
    return rollingSummaryData;
}
