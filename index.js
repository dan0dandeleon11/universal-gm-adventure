...
import {
    ...
    testGMAPIConnection,
    saveGMApiKey
} from './src/systems/gm/index.js';
import { createGMModeTabHTML, setupGMModeEvents, refreshGMModeDisplay, cleanupGMModeUI } from './src/systems/gm/gmModeUI.js';
import { injectGMRuling, clearGMInjection, enableUnifiedMode, isUnifiedModeEnabled } from './src/systems/gm/gmInjection.js';

// Summarizer modules
import { maybeRefreshSummary, clearSummaryInjection } from './src/systems/summarizer/index.js';

// Co-Op Relay modules
import { onGMTurnComplete } from './src/systems/coop/index.js';

// Feature modules (new)
import { runMemoryRecollection, setupMemoryRecollectionButton } from './src/systems/features/memoryRecollection.js';
import { applyLorebookLimit, isLorebookLimiterEnabled } from './src/systems/features/lorebookLimiter.js';

// Pipeline modules
import { clearPipelineState } from './src/systems/pipeline/index.js';

// Agent initializers
import { initNarrativeDirector } from './src/systems/agents/narrativeDirector.js';
import { initWorldStateAgent } from './src/systems/agents/worldState.js';
import { initSceneModel } from './src/systems/scene/index.js';
import { initNPCRouter } from './src/systems/agents/npcRouter.js';
...
async function addExtensionSettings() {
    ...
    // GM Mode settings
    $('#rpg-gm-enabled').prop('checked', extensionSettings.gmMode?.enabled || false).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        extensionSettings.gmMode.enabled = $(this).prop('checked');
        saveSettings();
        if (extensionSettings.gmMode.enabled) {
            $('#rpg-gm-tab-container').show();
            setupGMModeEvents();
            refreshGMModeDisplay();
        } else {
            $('#rpg-gm-tab-container').hide();
            cleanupGMModeUI();
            clearGMInjection();
        }
    });

    $('#rpg-gm-unified').prop('checked', extensionSettings.gmMode?.unifiedMode || false).on('change', async function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        extensionSettings.gmMode.unifiedMode = $(this).prop('checked');
        saveSettings();
        if (extensionSettings.gmMode.unifiedMode) {
    ...
    // Summarizer settings
    $('#rpg-summarizer-enabled').prop('checked', extensionSettings.summarizer?.enabled || false).on('change', function() {
        if (!extensionSettings.summarizer) extensionSettings.summarizer = {};
        extensionSettings.summarizer.enabled = $(this).prop('checked');
        saveSettings();
        if (!extensionSettings.summarizer.enabled) clearSummaryInjection();
    });

    $('#rpg-summarizer-strategy').val(extensionSettings.summarizer?.strategy || 'fixed').on('change', function() {
        if (!extensionSettings.summarizer) extensionSettings.summarizer = {};
        extensionSettings.summarizer.strategy = String($(this).val());
        saveSettings();
    });

    $('#rpg-summarizer-depth').val(extensionSettings.summarizer?.summaryDepth ?? 10).on('change', function() {
        if (!extensionSettings.summarizer) extensionSettings.summarizer = {};
        extensionSettings.summarizer.summaryDepth = parseInt(String($(this).val()));
        saveSettings();
    });

    // Co-Op Relay settings
    $('#rpg-coop-enabled').prop('checked', extensionSettings.coopRelay?.enabled || false).on('change', function() {
        if (!extensionSettings.coopRelay) extensionSettings.coopRelay = {};
        extensionSettings.coopRelay.enabled = $(this).prop('checked');
        saveSettings();
        $('#rpg-coop-relay-output').toggle(extensionSettings.coopRelay.enabled);
    });

    $('#rpg-coop-auto-clipboard').prop('checked', extensionSettings.coopRelay?.autoClipboard || false).on('change', function() {
        if (!extensionSettings.coopRelay) extensionSettings.coopRelay = {};
        extensionSettings.coopRelay.autoClipboard = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-coop-partner-name').val(extensionSettings.coopRelay?.partnerName || 'Caleb').on('change', function() {
        if (!extensionSettings.coopRelay) extensionSettings.coopRelay = {};
        extensionSettings.coopRelay.partnerName = String($(this).val());
        saveSettings();
    });

    // Memory Recollection settings
    $('#rpg-memory-enabled').prop('checked', extensionSettings.memoryRecollection?.enabled || false).on('change', function() {
        if (!extensionSettings.memoryRecollection) extensionSettings.memoryRecollection = {};
        extensionSettings.memoryRecollection.enabled = $(this).prop('checked');
        saveSettings();
        if (extensionSettings.memoryRecollection.enabled) setupMemoryRecollectionButton();
    });

    $('#rpg-memory-batch-size').val(extensionSettings.memoryRecollection?.batchSize || 10).on('change', function() {
        if (!extensionSettings.memoryRecollection) extensionSettings.memoryRecollection = {};
        extensionSettings.memoryRecollection.batchSize = parseInt(String($(this).val()));
        saveSettings();
    });

    $('#rpg-memory-lorebook').val(extensionSettings.memoryRecollection?.lorebookName || 'Memory Recollection').on('change', function() {
        if (!extensionSettings.memoryRecollection) extensionSettings.memoryRecollection = {};
        extensionSettings.memoryRecollection.lorebookName = String($(this).val());
        saveSettings();
    });

    // Lorebook Limiter settings
    $('#rpg-lorebook-limiter-enabled').prop('checked', extensionSettings.lorebookLimiter?.enabled || false).on('change', function() {
        if (!extensionSettings.lorebookLimiter) extensionSettings.lorebookLimiter = {};
        extensionSettings.lorebookLimiter.enabled = $(this).prop('checked');
        saveSettings();
        if (isLorebookLimiterEnabled()) applyLorebookLimit();
    });

    $('#rpg-lorebook-max-activations').val(extensionSettings.lorebookLimiter?.maxActivations || 10).on('change', function() {
        if (!extensionSettings.lorebookLimiter) extensionSettings.lorebookLimiter = {};
        extensionSettings.lorebookLimiter.maxActivations = parseInt(String($(this).val()));
        saveSettings();
    });

    // Pipeline settings
    $('#rpg-pipeline-enabled').prop('checked', extensionSettings.pipeline?.enabled || false).on('change', function() {
        if (!extensionSettings.pipeline) extensionSettings.pipeline = {};
        extensionSettings.pipeline.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-pipeline-timeout').val(extensionSettings.pipeline?.agentTimeout || 60000).on('change', function() {
        if (!extensionSettings.pipeline) extensionSettings.pipeline = {};
        extensionSettings.pipeline.agentTimeout = parseInt(String($(this).val()));
        saveSettings();
    });

    // Narrative Director settings
    $('#rpg-narrative-director-enabled').prop('checked', extensionSettings.narrativeDirector?.enabled || false).on('change', function() {
        if (!extensionSettings.narrativeDirector) extensionSettings.narrativeDirector = {};
        extensionSettings.narrativeDirector.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-narrative-director-mode').val(extensionSettings.narrativeDirector?.mode || 'natural').on('change', function() {
        if (!extensionSettings.narrativeDirector) extensionSettings.narrativeDirector = {};
        extensionSettings.narrativeDirector.mode = String($(this).val());
        saveSettings();
    });

    $('#rpg-narrative-secret-plot').prop('checked', extensionSettings.narrativeDirector?.secretPlotEnabled || false).on('change', function() {
        if (!extensionSettings.narrativeDirector) extensionSettings.narrativeDirector = {};
        extensionSettings.narrativeDirector.secretPlotEnabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-narrative-secret-interval').val(extensionSettings.narrativeDirector?.secretPlotRunInterval || 8).on('change', function() {
        if (!extensionSettings.narrativeDirector) extensionSettings.narrativeDirector = {};
        extensionSettings.narrativeDirector.secretPlotRunInterval = parseInt(String($(this).val()));
        saveSettings();
    });

    // World State Agent settings
    $('#rpg-world-state-enabled').prop('checked', extensionSettings.worldStateAgent?.enabled || false).on('change', function() {
        if (!extensionSettings.worldStateAgent) extensionSettings.worldStateAgent = {};
        extensionSettings.worldStateAgent.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-world-state-inject').prop('checked', extensionSettings.worldStateAgent?.injectAsContext !== false).on('change', function() {
        if (!extensionSettings.worldStateAgent) extensionSettings.worldStateAgent = {};
        extensionSettings.worldStateAgent.injectAsContext = $(this).prop('checked');
        saveSettings();
    });

    // NPC System settings
    $('#rpg-npc-enabled').prop('checked', extensionSettings.npcSystem?.enabled || false).on('change', function() {
        if (!extensionSettings.npcSystem) extensionSettings.npcSystem = {};
        extensionSettings.npcSystem.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-npc-max-tracked').val(extensionSettings.npcSystem?.maxTracked || 50).on('change', function() {
        if (!extensionSettings.npcSystem) extensionSettings.npcSystem = {};
        extensionSettings.npcSystem.maxTracked = parseInt(String($(this).val()));
        saveSettings();
    });

    $('#rpg-npc-inject-relevant').prop('checked', extensionSettings.npcSystem?.injectRelevant !== false).on('change', function() {
        if (!extensionSettings.npcSystem) extensionSettings.npcSystem = {};
        extensionSettings.npcSystem.injectRelevant = $(this).prop('checked');
        saveSettings();
    });

    // NPC Router settings
    $('#rpg-npc-router-enabled').prop('checked', extensionSettings.npcRouter?.enabled || false).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-npc-router-max-per-turn').val(extensionSettings.npcRouter?.maxNPCsPerTurn || 3).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.maxNPCsPerTurn = parseInt(String($(this).val()));
        saveSettings();
    });

    $('#rpg-npc-router-autonomy').val(extensionSettings.npcRouter?.autonomy || 'reactive').on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.autonomy = String($(this).val());
        saveSettings();
    });

    $('#rpg-npc-router-rep-deltas').prop('checked', extensionSettings.npcRouter?.applyReputationDeltas !== false).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.applyReputationDeltas = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-npc-router-inject').prop('checked', extensionSettings.npcRouter?.injectResponses !== false).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.injectResponses = $(this).prop('checked');
        saveSettings();
    });

    // NPC Router connection settings
    const npcRouterUseGm = extensionSettings.npcRouter?.useGmConnection !== false;
    $('#rpg-npc-router-use-gm').prop('checked', npcRouterUseGm).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.useGmConnection = $(this).prop('checked');
        $('#rpg-npc-router-connection-fields').toggle(!$(this).prop('checked'));
        saveSettings();
    });
    $('#rpg-npc-router-connection-fields').toggle(!npcRouterUseGm);

    const npcConnSettings = extensionSettings.npcRouter?.settings?.custom || {};
    $('#rpg-npc-router-endpoint').val(npcConnSettings.endpoint || '').on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        if (!extensionSettings.npcRouter.settings) extensionSettings.npcRouter.settings = {};
        if (!extensionSettings.npcRouter.settings.custom) extensionSettings.npcRouter.settings.custom = {};
        extensionSettings.npcRouter.settings.custom.endpoint = String($(this).val()).trim();
        extensionSettings.npcRouter.provider = 'custom';
        saveSettings();
    });

    $('#rpg-npc-router-model').val(npcConnSettings.model || '').on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        if (!extensionSettings.npcRouter.settings) extensionSettings.npcRouter.settings = {};
        if (!extensionSettings.npcRouter.settings.custom) extensionSettings.npcRouter.settings.custom = {};
        extensionSettings.npcRouter.settings.custom.model = String($(this).val()).trim();
        saveSettings();
    });

    $('#rpg-npc-router-temperature').val(npcConnSettings.temperature ?? 0.7).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        if (!extensionSettings.npcRouter.settings) extensionSettings.npcRouter.settings = {};
        if (!extensionSettings.npcRouter.settings.custom) extensionSettings.npcRouter.settings.custom = {};
        extensionSettings.npcRouter.settings.custom.temperature = parseFloat($(this).val());
        saveSettings();
    });

    $('#rpg-npc-router-max-tokens').val(extensionSettings.npcRouter?.maxTokens || 500).on('change', function() {
        if (!extensionSettings.npcRouter) extensionSettings.npcRouter = {};
        extensionSettings.npcRouter.maxTokens = parseInt(String($(this).val()));
        saveSettings();
    });

    // API key stored in localStorage, not in settings (security)
    // Falls back to GM API key (rpg_companion_gm_api_key_custom) if not set
    $('#rpg-npc-router-api-key').val(localStorage.getItem('rpg_companion_npcRouter_api_key') || '').on('change', function() {
        const key = String($(this).val()).trim();
        if (key) {
            localStorage.setItem('rpg_companion_npcRouter_api_key', key);
        } else {
            localStorage.removeItem('rpg_companion_npcRouter_api_key');
        }
    });

    // Session Management settings
    $('#rpg-sessions-enabled').prop('checked', extensionSettings.sessions?.enabled || false).on('change', function() {
        if (!extensionSettings.sessions) extensionSettings.sessions = {};
        extensionSettings.sessions.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-sessions-auto-summary').prop('checked', extensionSettings.sessions?.autoSummaryOnEnd !== false).on('change', function() {
        if (!extensionSettings.sessions) extensionSettings.sessions = {};
        extensionSettings.sessions.autoSummaryOnEnd = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-sessions-carry-depth').val(extensionSettings.sessions?.carryForwardDepth || 3).on('change', function() {
        if (!extensionSettings.sessions) extensionSettings.sessions = {};
        extensionSettings.sessions.carryForwardDepth = parseInt(String($(this).val()));
        saveSettings();
    });

    // Campaign Planning settings
    $('#rpg-campaign-enabled').prop('checked', extensionSettings.campaign?.enabled || false).on('change', function() {
        if (!extensionSettings.campaign) extensionSettings.campaign = {};
        extensionSettings.campaign.enabled = $(this).prop('checked');
        saveSettings();
    });

    // Maps settings
    $('#rpg-maps-enabled').prop('checked', extensionSettings.maps?.enabled || false).on('change', function() {
        if (!extensionSettings.maps) extensionSettings.maps = {};
        extensionSettings.maps.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-maps-type').val(extensionSettings.maps?.type || 'overworld').on('change', function() {
        if (!extensionSettings.maps) extensionSettings.maps = {};
        extensionSettings.maps.type = String($(this).val());
        saveSettings();
    });

    $('#rpg-maps-fog').prop('checked', extensionSettings.maps?.fogOfWar !== false).on('change', function() {
        if (!extensionSettings.maps) extensionSettings.maps = {};
        extensionSettings.maps.fogOfWar = $(this).prop('checked');
        saveSettings();
    });

    // Scene Model settings
    $('#rpg-scene-enabled').prop('checked', extensionSettings.sceneModel?.enabled || false).on('change', function() {
        if (!extensionSettings.sceneModel) extensionSettings.sceneModel = {};
        extensionSettings.sceneModel.enabled = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-scene-use-gm').prop('checked', extensionSettings.sceneModel?.useGmConnection !== false).on('change', function() {
        if (!extensionSettings.sceneModel) extensionSettings.sceneModel = {};
        extensionSettings.sceneModel.useGmConnection = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-scene-backgrounds').prop('checked', extensionSettings.sceneModel?.generateBackgrounds !== false).on('change', function() {
        if (!extensionSettings.sceneModel) extensionSettings.sceneModel = {};
        extensionSettings.sceneModel.generateBackgrounds = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-scene-music').prop('checked', extensionSettings.sceneModel?.generateMusic || false).on('change', function() {
        if (!extensionSettings.sceneModel) extensionSettings.sceneModel = {};
        extensionSettings.sceneModel.generateMusic = $(this).prop('checked');
        saveSettings();
    });

    $('#rpg-scene-sfx').prop('checked', extensionSettings.sceneModel?.generateSFX || false).on('change', function() {
        if (!extensionSettings.sceneModel) extensionSettings.sceneModel = {};
        extensionSettings.sceneModel.generateSFX = $(this).prop('checked');
        saveSettings();
    });
}
...
async function initUI() {
    ...
    // Initialize GM Mode UI if enabled
    if (extensionSettings.gmMode?.enabled) {
        try {
            $('#rpg-gm-tab-container').show();
            setupGMModeEvents();
            refreshGMModeDisplay();
            console.log('[RPG Companion] GM Mode UI initialized');
        } catch (error) {
            console.error('[RPG Companion] GM Mode UI initialization failed:', error);
        }
    }

    // Show co-op relay output area if enabled
    if (extensionSettings.coopRelay?.enabled) {
        $('#rpg-coop-relay-output').show();
    }

    // Initialize unified mode if configured
    if (extensionSettings.gmMode?.unifiedMode) {
        try {
            await enableUnifiedMode();
            console.log('[RPG Companion] Unified context mode enabled');
        } catch (error) {
            console.error('[RPG Companion] Unified mode initialization failed:', error);
        }
    }

    // Initialize summarizer if enabled
    if (extensionSettings.summarizer?.enabled) {
        try {
            console.log('[RPG Companion] Summarizer enabled');
        } catch (error) {
            console.error('[RPG Companion] Summarizer initialization failed:', error);
        }
    }

    // Initialize memory recollection button if enabled
    if (extensionSettings.memoryRecollection?.enabled) {
        try {
            setupMemoryRecollectionButton();
            console.log('[RPG Companion] Memory recollection button initialized');
        } catch (error) {
            console.error('[RPG Companion] Memory recollection initialization failed:', error);
        }
    }

    // Initialize lorebook limiter if enabled
    if (isLorebookLimiterEnabled()) {
        try {
            applyLorebookLimit();
            console.log('[RPG Companion] Lorebook limiter initialized');
        } catch (error) {
            console.error('[RPG Companion] Lorebook limiter initialization failed:', error);
        }
    }

    // Register pipeline agents
    try {
        initNarrativeDirector();
        initWorldStateAgent();
        initSceneModel();
        initNPCRouter();
        console.log('[RPG Companion] Pipeline agents registered');
    } catch (error) {
        console.error('[RPG Companion] Pipeline agent registration failed:', error);
    }
    ...
    try {
        ...
        try {
            registerAllEvents({
                ...
                [event_types.CHAT_CHANGED]: [onCharacterChanged, updatePersonaAvatar, restoreCheckpointOnLoad, clearSessionAvatarPrompts, clearPipelineState],
