...
import {
    ...
    // GM Mode state
    getPendingGMRoll,
    setPendingGMRoll as setGMPendingRoll,
...
// GM Mode modules (Universe GM Integration)
import {
    rollForGM,
    performRoll,
    formatRollResult,
    ...
    parseDiceNotation,
    peekPendingGMRoll
} from './src/systems/gm/dice.js';
import {
    getAvailableFrameworks,
    getActiveFramework,
    setActiveFramework,
    hasFeature
} from './src/systems/gm/frameworks.js';
import {
    processGMTurn,
    testGMAPIConnection,
    saveGMApiKey
} from './src/systems/gm/gmEngine.js';
import {
    injectGMRuling,
    clearGMInjection,
    previewInjection,
...
/**
 * Initialize GM Mode UI and event handlers
 */
function initGMModeUI() {
    ...
    if (gmSettings.enabled) {
        $('#rpg-gm-mode-section').show();
        $('#rpg-gm-controls').show();
    }

    // GM Mode enable toggle in main panel
    $('#rpg-gm-mode-enabled').prop('checked', gmSettings.enabled || false).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        extensionSettings.gmMode.enabled = $(this).prop('checked');
        saveSettings();
        $('#rpg-gm-controls').toggle(extensionSettings.gmMode.enabled);
    });

    // Framework selector
    $('#rpg-gm-framework').val(gmSettings.activeFramework || 'generic').on('change', function() {
    ...
    $('#rpg-gm-roll-d20').on('click', function() {
        const action = $('#rpg-gm-roll-action').val() || '';
        const roll = rollForGM(1, 20, 0, action);
        updateGMPendingRollDisplay(roll);
    });

    // Custom roll button - opens the dice modal
    $('#rpg-gm-roll-custom').on('click', function() {
        $('#rpg-dice-popup').show();
    });

    // Clear pending roll
    $('#rpg-gm-clear-roll').on('click', function() {
        setGMPendingRoll(null);
        $('#rpg-gm-pending-roll').hide();
    });

    // Process GM turn button
    $('#rpg-gm-process-turn').on('click', async function() {
        const $btn = $(this);
        const originalHtml = $btn.html();
        ...
        try {
            // Get the user's message from the input area
            const userMessage = $('#send_textarea').val() || '';
            ...
            if (result.success) {
                // Show narration preview
                $('#rpg-gm-narration-content').text(result.narration);
                $('#rpg-gm-narration-preview').show();

                // Clear pending roll display (it's been consumed)
                $('#rpg-gm-pending-roll').hide();
    ...
    $('#rpg-gm-clear-narration').on('click', function() {
        clearGMInjection();
        $('#rpg-gm-narration-preview').hide();
    });

    // === GM Settings Modal Handlers ===

    // GM Mode settings toggle (in settings modal)
    $('#rpg-toggle-gm-mode-settings').prop('checked', gmSettings.enabled || false).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        extensionSettings.gmMode.enabled = $(this).prop('checked');
        saveSettings();
    ...
    // Injection mode
    const injectionSettings = gmSettings.injection || {};
    $('#rpg-gm-injection-mode').val(injectionSettings.mode || 'ambient').on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.injection) extensionSettings.gmMode.injection = {};
        extensionSettings.gmMode.injection.mode = $(this).val();
        saveSettings();
    });

    // Injection position
    $('#rpg-gm-injection-position').val(injectionSettings.position || 'before_user_message').on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.injection) extensionSettings.gmMode.injection = {};
        extensionSettings.gmMode.injection.position = $(this).val();
        saveSettings();
    });

    // Custom tag name
    const customTags = injectionSettings.customTags || {};
    $('#rpg-gm-tag-name').val(customTags.gmRuling || '').on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.injection) extensionSettings.gmMode.injection = {};
        if (!extensionSettings.gmMode.injection.customTags) extensionSettings.gmMode.injection.customTags = {};
        extensionSettings.gmMode.injection.customTags.gmRuling = $(this).val().trim();
        saveSettings();
    });

    // Dice thresholds
    const diceSettings = gmSettings.dice || {};
    const thresholds = diceSettings.thresholds || {};
    $('#rpg-gm-threshold-crit-fail').val(thresholds.critFail || 1).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.dice) extensionSettings.gmMode.dice = {};
        if (!extensionSettings.gmMode.dice.thresholds) extensionSettings.gmMode.dice.thresholds = {};
        extensionSettings.gmMode.dice.thresholds.critFail = parseInt($(this).val());
        saveSettings();
    });
    $('#rpg-gm-threshold-fail').val(thresholds.fail || 5).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.dice) extensionSettings.gmMode.dice = {};
        if (!extensionSettings.gmMode.dice.thresholds) extensionSettings.gmMode.dice.thresholds = {};
        extensionSettings.gmMode.dice.thresholds.fail = parseInt($(this).val());
        saveSettings();
    });
    $('#rpg-gm-threshold-partial').val(thresholds.partial || 10).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.dice) extensionSettings.gmMode.dice = {};
        if (!extensionSettings.gmMode.dice.thresholds) extensionSettings.gmMode.dice.thresholds = {};
        extensionSettings.gmMode.dice.thresholds.partial = parseInt($(this).val());
        saveSettings();
    });
    $('#rpg-gm-threshold-success').val(thresholds.success || 15).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.dice) extensionSettings.gmMode.dice = {};
        if (!extensionSettings.gmMode.dice.thresholds) extensionSettings.gmMode.dice.thresholds = {};
        extensionSettings.gmMode.dice.thresholds.success = parseInt($(this).val());
        saveSettings();
    });

    // GM API settings
    const apiSettings = gmSettings.api || {};
    const providerSettings = apiSettings.settings?.custom || {};

    $('#rpg-gm-api-endpoint').val(providerSettings.endpoint || '').on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.api) extensionSettings.gmMode.api = { provider: 'custom', settings: {} };
        if (!extensionSettings.gmMode.api.settings) extensionSettings.gmMode.api.settings = {};
        if (!extensionSettings.gmMode.api.settings.custom) extensionSettings.gmMode.api.settings.custom = {};
        extensionSettings.gmMode.api.settings.custom.endpoint = $(this).val().trim();
        saveSettings();
    });

    // Load GM API key from localStorage
    const gmApiKey = localStorage.getItem('rpg_companion_gm_api_key_custom') || '';
    $('#rpg-gm-api-key').val(gmApiKey).on('change', function() {
        const key = $(this).val().trim();
        saveGMApiKey('custom', key);
    });

    $('#rpg-gm-toggle-api-key').on('click', function() {
        const $input = $('#rpg-gm-api-key');
        const type = $input.attr('type') === 'password' ? 'text' : 'password';
        $input.attr('type', type);
        $(this).find('i').toggleClass('fa-eye fa-eye-slash');
    });

    $('#rpg-gm-api-model').val(providerSettings.model || '').on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.api) extensionSettings.gmMode.api = { provider: 'custom', settings: {} };
        if (!extensionSettings.gmMode.api.settings) extensionSettings.gmMode.api.settings = {};
        if (!extensionSettings.gmMode.api.settings.custom) extensionSettings.gmMode.api.settings.custom = {};
        extensionSettings.gmMode.api.settings.custom.model = $(this).val().trim();
        saveSettings();
    });

    $('#rpg-gm-api-max-tokens').val(apiSettings.maxTokens || 1000).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.api) extensionSettings.gmMode.api = {};
        extensionSettings.gmMode.api.maxTokens = parseInt($(this).val());
        saveSettings();
    });

    $('#rpg-gm-api-temperature').val(providerSettings.temperature ?? 0.8).on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.api) extensionSettings.gmMode.api = { provider: 'custom', settings: {} };
        if (!extensionSettings.gmMode.api.settings) extensionSettings.gmMode.api.settings = {};
        if (!extensionSettings.gmMode.api.settings.custom) extensionSettings.gmMode.api.settings.custom = {};
        extensionSettings.gmMode.api.settings.custom.temperature = parseFloat($(this).val());
        saveSettings();
    });

    // Test GM API connection
    $('#rpg-gm-test-api').on('click', async function() {
        const $btn = $(this);
        const $result = $('#rpg-gm-api-test-result');
        const originalHtml = $btn.html();
        ...
        try {
            const result = await testGMAPIConnection();
    ...
    // Injection template
    const templates = injectionSettings.templates || {};
    $('#rpg-gm-injection-template').val(templates.gmRuling || '').on('change', function() {
        if (!extensionSettings.gmMode) extensionSettings.gmMode = {};
        if (!extensionSettings.gmMode.injection) extensionSettings.gmMode.injection = {};
        if (!extensionSettings.gmMode.injection.templates) extensionSettings.gmMode.injection.templates = {};
        extensionSettings.gmMode.injection.templates.gmRuling = $(this).val();
        saveSettings();
    });

    // Preview template
    $('#rpg-gm-template-preview').on('click', function() {
        const sampleNarration = 'The dice clatter across the table. You attempt to pick the lock, but your tools slip - the mechanism remains stubbornly closed.';
        const sampleRoll = { total: 7, modifiedTotal: 7, result: 'fail', description: 'Failure', formula: '1d20' };
        const preview = previewInjection(sampleNarration, sampleRoll);
        $('#rpg-gm-template-preview-output').text(preview).slideDown();
    });

    // Reset template to default
    $('#rpg-gm-template-reset').on('click', function() {
        const defaults = getDefaultTemplates();
        $('#rpg-gm-injection-template').val('');
        if (extensionSettings.gmMode?.injection?.templates) {
            delete extensionSettings.gmMode.injection.templates.gmRuling;
        }
        saveSettings();
    ...
    $('#rpg-gm-mode-section').toggle(gmSettings.enabled || false);
    $('#rpg-gm-controls').toggle(gmSettings.enabled || false);

    console.log('[RPG Companion GM] GM Mode UI initialized');
}

/**
 * Update the pending GM roll display in the panel
 */
function updateGMPendingRollDisplay(roll) {
    if (!roll) {
        $('#rpg-gm-pending-roll').hide();
        return;
    }

    const emoji = getResultEmoji(roll.result);
    const text = formatRollResult(roll);

    $('#rpg-gm-roll-emoji').text(emoji);
    $('#rpg-gm-roll-text').text(text);
    $('#rpg-gm-pending-roll')
        .removeClass('rpg-gm-roll-crit-fail rpg-gm-roll-fail rpg-gm-roll-partial rpg-gm-roll-success rpg-gm-roll-crit-success')
        .addClass(`rpg-gm-roll-${roll.result}`)
        .show();
}
